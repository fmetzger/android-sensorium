...
### Guten Tag und danke, da&szlig; Sie vorbeischauen! ###
...
Die App speichert Sensorwerte (3G, GPS, Akkustand usw.), die Sie uns erlauben auszulesen; Sie k&ouml;nnen die Aufl&ouml;sung der gesammelten Daten in der App beliebig steuern. Die oben verlinkte Version der App ist vorkonfiguriert, Daten f&uuml;r unsere offene 3G-Netzabdeckungskarte [,,Open3GMap''][102] zu sammeln. Daf&uuml;r speichern wir Ihre aktuelle GPS-Position sowie einige Details &uuml;ber die Empfang&szlig;t&auml;rke und Verbindungstechnologie Ihrer Mobilnetz-Anbindung auf unserem Server (falls Sie diese Daten zur Verf&uuml;gung stellen wollen). Weitere Details zum Datenschutz finden Sie im Folgenden.

Danke f&uuml;r Ihr Intere&szlig;e! Wir freuen uns auf Ihren Besuch bei unserem Demo-Stand.
...
Relevante Datenpunkte f&uuml;r die Netzabdeckungskarte sind GPS-Koordinaten sowie die Daten des 3G-Interfaces (MCC, MNC und LAC, Cell ID, Verbindungstechnologie und R&szlig;I, Operator und Roaming-Status), jeweils in voller Genauigkeit. Wir erfa&szlig;en weder Ihre Telefonnummer (IMSI) noch Telefon-Seriennummer (IMEI) noch IP-Adre&szlig;e. Die Daten werden um der Freiwilligkeit des Zur-Verf&uuml;gung-Stellens Rechnung zu tragen als Open Data auf unserem Server zum Download angeboten. Bitte beachten Sie die m&ouml;glichen Konsequenzen f&uuml;r Ihre Privatsph&auml;re (und die anderer Leute).
...
The app collects sensor values (such as 3G, GPS, battery charge) that you allow us to acce&szlig;. You can control the level of detail of the data collected from within the app. The app comes preconfigured for collecting data for our open 3G connectivity map, [``Open3GMap''][102]. For this, we store your GPS location and details about current 3G coverage such as signal strength and acce&szlig; technology on our server (in case you choose to share this data with us). If you care about privacy, be sure to read on.
...
* Sanitized sensor values are made acce&szlig;ible to other applications running on the same device over an XML-RPC interfac if you configure Sensorium to do so. We will present an example of this using the [Seattle][103] testbed software during our demo.
...
Values we need for the map display are GPS coordinates and data from the 3G interface (MCC, MNC, and LAC, Cell ID, acce&szlig; technology, and R&szlig;I, mobile operator and roaming status), all at the highest level of detail. We do not record your phone number (IMSI) nor phone serial number (IMEI) nor IP addre&szlig;. To acknowledge our volunteer contributors, we make the collected data available for download as Open Data. Please keep the effects on your (and anybody else's) privacy in mind.
We also collect other sanitized sensor data if you happen to share them. We do not make these public, but use them as a playground for algorithms that deal with incomplete or le&szlig; accurate data.
